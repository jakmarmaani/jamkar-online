...
state.paid=false;try{localStorage.removeItem('jamkar_paid')}catch{}renderStats();renderWorlds();
...
function mergeAchievements(rows){const all=(()=>{try{return JSON.parse(localStorage.getItem('jamkar_achievements')||'{}')}catch{return{}}})();const profile=state.activeProfile;all[profile]=all[profile]||[];for(const r of rows||[])if(r.achievement_key&&!all[profile].includes(r.achievement_key))all[profile].push(r.achievement_key);localStorage.setItem('jamkar_achievements',JSON.stringify(all))}
async function pullCloud(){
 ...
 family=await backend.getFamily();state.paid=!!family?.account?.lifetime_unlocked;
 const names=(family?.children||[]).map(c=>c.username);if(names.length){localStorage.setItem('jamkar_profiles',JSON.stringify(names));if(!names.includes(state.activeProfile))state.activeProfile=names[0];loadProfiles()}
 const child=currentChild();if(child){const [rows,achievements]=await Promise.all([backend.loadProgress(child.id),backend.loadAchievements(child.id)]);rows.forEach(r=>{state.progress[state.activeProfile+'|'+r.world_key]=Math.max(1,Number(r.highest_level||1))});localStorage.setItem('jamkar_progress',JSON.stringify(state.progress));mergeAchievements(achievements)}
...
window.addEventListener('jamkar:achievement',e=>{if(!configured()||!backend.state.user)return;const child=currentChild();const key=e.detail?.name;if(child&&key)backend.saveAchievement(child.id,key).catch(err=>console.error('Cloud achievement sync failed',err))});
