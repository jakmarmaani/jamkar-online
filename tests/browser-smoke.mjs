...
async function desktopSmoke(){
 ...
 await activate(page.locator('#levelGrid .level').first());await page.waitForTimeout(450);if(!(await page.locator('#gameStage').textContent())?.trim())throw new Error('Game stage did not render');
...
async function mobileSmoke(){
 ...
 await activate(page.locator('#worlds .worldCard').first());await page.waitForSelector('#worldModal.open');await activate(page.locator('#levelGrid .level').first());await page.waitForTimeout(450);
...
console.log(`${browserName} browser smoke passed: redesigned desktop/mobile runtime, floating game cards, paywall tamper resistance, themed gameplay, music, navigation and Parent Area.`);
