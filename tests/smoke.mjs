...
const immersive=read('immersive-game.css');
const adventure=read('adventure-runtime.js');
const adventureCss=read('adventure-runtime.css');
...
ok('Child-first light theme loaded',index.includes('sunny-theme.css')&&index.includes('home-experience.js')&&/cardFloat/.test(sunny));
ok('Full-screen world shell loaded last',index.includes('immersive-game.css')&&index.indexOf('immersive-game.css')>index.indexOf('sunny-theme.css')&&/100dvh/.test(immersive)&&/overflow-x:auto/.test(immersive));
ok('World launches directly into current level',/requestAnimationFrame\(\(\)=>\{startLevel\(state\.selectedLevel\)/.test(app));
ok('Visible Back navigation and browser history integration',/worldBack/.test(immersive)&&/history\.pushState/.test(home)&&/popstate/.test(home));
ok('Adventure runtime is primary playable surface',index.includes('adventure-runtime.js')&&index.includes('adventure-runtime.css')&&/adventurePlayer/.test(adventure)&&/pointer-events:auto/.test(adventureCss));
ok('Primitive hero WebGL removed from homepage',!index.includes('webgl-world.js')&&!index.includes('webgl-world.css')&&index.includes('mascotScene'));
...
ok('Admin magic-link bootstrap configured',adminHtml.includes('adminMagic')&&/signInWithOtp/.test(admin)&&admin.includes('mohamed.a.elmonim@gmail.com'));
ok('Admin can create invited family accounts without payment',adminHtml.includes('createFamily')&&/admin-create-family/.test(admin));
...
ok('WebGL gameplay code remains available behind polished presentation',index.includes('webgl-characters.js')&&index.includes('webgl-environments.js')&&/getContext\('webgl'/.test(characters));
ok('Interactive 3D mission logic retained',index.includes('world-navigation.js')&&/JamKar3DPlayer/.test(navigation)&&/WASD/.test(navigation));
